...
//----------------------------------------------------------------------------------------
  ...
  if(argc < 4)
  {
    cout << " Usage: " << appName << " config_file" << " lhe_file" << " output_file" << endl;
...
// do the simulation
  ...
  try
  {
    ...
    for(eventCounter = 0; !interrupted; ++eventCounter)
    {
      ...
      readStopWatch.Start();
      progressBar.Update(eventCounter-startEvent, eventCounter-startEvent);
    }

    progressBar.Update(eventCounter-startEvent, eventCounter-startEvent, kTRUE);
    progressBar.Finish();

    cout << "############### total simulated event:   " << eventCounter-startEvent << "   Fail:   " << errorCounter << "   ###############" << endl;
