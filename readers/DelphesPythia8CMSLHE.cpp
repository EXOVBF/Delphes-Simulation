...
bool lhe_event_preselection(vector< vector<float> >* LHE_event, float Mjj_cut, vector<ExRootTreeBranch*> branchVector)
{
    ...
    for(unsigned int iPart = 0; iPart < LHE_event->size(); iPart++)
    {
    ...
    for(unsigned int iPartons = 0; iPartons < outPartons.size(); iPartons++)
    {
        TLorentzVector tmp4vect = outPartons.at(iPartons);
        for(unsigned int jQuark = iPartons+1; jQuark < outPartons.size(); jQuark++)
        {
...
// do the simulation
	...
	//pythia->readString(sfile.c_str()); 
        //pythia->init();// will first need to create the LHE header from the root file
        //------------------------------------------------------------------------------------------
        //-----  root file -----
	...
	// get initilazaiton info
	fwlite::Handle<LHERunInfoProduct> handleinfo;
	handleinfo.getByLabel(run,"externalLHEProducer");
	const lhef::HEPRUP &heprup = handleinfo->heprup();
	
	genreader.loadRunInfo(heprup);
	genreader.setInit();

	// //create LHE header to start pythia initialization
	// std::string fnam = inputFile+".lheheader";
	// std::fstream header;
	// header.open(fnam,std::fstream::out |std::fstream::trunc );
	// header << "<LesHouchesEvents version=\"1.0\">" <<std::endl;
	// header << "<init>" << std::endl;
	// header << scientific << setprecision(6);
	// header << heprup.IDBMUP.first << "  " <<  heprup.IDBMUP.second << "   " \
	//        << heprup.EBMUP.first << "  " <<  heprup.EBMUP.second  << "   " \
	//        << heprup.PDFGUP.first << "  " <<  heprup.PDFGUP.second  << "   " \
	//        << heprup.PDFSUP.first << "  " <<  heprup.PDFSUP.second  << "   " \
	//        << heprup.IDWTUP << "  " << heprup.NPRUP <<std::endl;
	// for(int i =0 ; i< heprup.NPRUP;i++)
	//   header  << heprup.XSECUP[i] << "  "	\
	// 	  << heprup.XERRUP[i] << "  "	\
	// 	  << heprup.XMAXUP[i] << "  "		\
	// 	  << heprup.LPRUP[i] << "  "  <<std::endl;

	// header << "</init>" <<std::endl;
	// header << "</LesHouchesEvents>" <<std::endl;
	// header.close();

	// std::string sfile = "Beams:LHEF ="+fnam;
	// pythia->readString(sfile.c_str());
	// pythia->settings.listAll();
        pythia->init(&genreader);// will first need to create the LHE header from the root file


	fwlite::Event event(inf);
	Long64_t numberOfEvents = event.size();
        Long64_t count=-1;
        Long64_t skippedCounter = 0;
	...
	const lhef::HEPEUP &hp = handlelhe->hepeup();
	genreader.loadEvent(hp);
        ...
        while(event)
	  {
	    ...
	    const lhef::HEPEUP &hepeup = handlelhe->hepeup();	    
		...
		LHE_particle.push_back(hepeup.ISTUP[i]);
		LHE_particle.push_back(hepeup.MOTHUP[i].first);
		LHE_particle.push_back(hepeup.MOTHUP[i].second);
